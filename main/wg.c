...
esp_err_t wifi_init_netif(void) {
  ...
  err = esp_event_handler_instance_unregister(IP_EVENT, IP_EVENT_STA_GOT_IP,
                                              instance_got_ip);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "esp_event_handler_instance_unregister: %s",
             esp_err_to_name(err));
    goto fail;
  }
...
static void test_on_ping_success(esp_ping_handle_t hdl, void *args) {
  ...
  esp_ping_get_profile(hdl, ESP_PING_PROF_TIMEGAP, &elapsed_time,
                       sizeof(elapsed_time));
  ESP_LOGI(TAG,
           "%" PRIu32 " bytes from %s icmp_seq=%" PRIu16 " ttl=%" PRIi8
           " time=%" PRIu32 " ms",
...
static void test_on_ping_timeout(esp_ping_handle_t hdl, void *args) {
  ...
  esp_ping_get_profile(hdl, ESP_PING_PROF_IPADDR, &target_addr,
                       sizeof(target_addr));
  ESP_LOGI(TAG, "From %s icmp_seq=%" PRIu16 " timeout",
...
static void test_on_ping_end(esp_ping_handle_t hdl, void *args) {
  ...
  ESP_LOGI(TAG,
           "%" PRIu32 " packets transmitted, %" PRIu32
           " received, time %" PRIu32 "ms",
...
void start_ping() {
  ...
  ESP_ERROR_CHECK(
      lwip_getaddrinfo(CONFIG_EXAMPLE_PING_ADDRESS, NULL, &hint, &res) == 0
          ? ESP_OK
          : ESP_FAIL);
